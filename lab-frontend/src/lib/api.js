...
// UI keys are intentionally separate from backend/provider ids used by the API.
// Keeping this explicit prevents the screen flow from silently selecting the wrong endpoint.
...
export function normalizeProvider(provider) {
  return PROVIDER_MAP[provider] || provider || null
}

function mapTestScope(scope) {
  return ({
    ts_quick: "quick",
...
function buildEvidenceStatus(form) {
  const provider = normalizeProvider(form.model_provider)
  const scope = mapTestScope(form.test_scope)
  const backendManagedProvider = ["openai", "anthropic"].includes(provider)
  const userKeyProvider = ["google", "huggingface", "custom"].includes(provider)

  if (!provider) {
    return {
      technical_test_requested: false,
      technical_test_status: "not_requested",
      technical_test_note: "No model provider was selected; risk score is declaration-based.",
    }
  }

  if (backendManagedProvider) {
    return {
      technical_test_requested: true,
      technical_test_status: "queued_for_backend_test",
      technical_test_scope: scope,
      technical_test_provider: provider,
      technical_test_note: "Backend-managed provider selected. Available evidence engines will be executed by /assess/risk/full.",
    }
  }

  if (userKeyProvider) {
    return {
      technical_test_requested: Boolean(form.api_key || form.custom_endpoint),
      technical_test_status: "not_configured_for_backend_execution",
      technical_test_scope: scope,
      technical_test_provider: provider,
      technical_test_note: "User-supplied provider metadata was captured, but this backend does not yet execute external user-key based tests. Risk score is declaration-based until a secure ephemeral execution path is implemented.",
    }
  }

  return {
    technical_test_requested: true,
    technical_test_status: "unsupported_provider",
    technical_test_scope: scope,
    technical_test_provider: provider,
  }
}
...
export function buildPayload(form) {
  ...
  return {
    ...
    technical_test_profile: {
      provider: normalizeProvider(form.model_provider),
      model_name: form.model_name || null,
      custom_endpoint: form.custom_endpoint || null,
      test_scope: mapTestScope(form.test_scope),
      user_api_key_supplied: Boolean(form.api_key),
      note: "API keys are intentionally not stored in the frontend payload. External user-key execution requires a secure ephemeral backend path.",
    },
  }
}
...
export async function runAssessment(form) {
  ...
  // Only backend-managed providers can safely trigger the current full-test endpoint.
  // User-provided external keys need a separate ephemeral execution service before use.
  const backendHasKey = ["openai", "anthropic"].includes(provider)

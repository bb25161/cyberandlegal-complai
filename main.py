...
class EvidenceLayer(BaseModel):
    ...
    technical_test_requested: bool = False
    technical_test_status: Optional[str] = None
    technical_test_scope: Optional[str] = None
    technical_test_provider: Optional[str] = None
    technical_test_note: Optional[str] = None


class EUScreeningProfile(BaseModel):
    role: Optional[str] = None
    risk_category: Optional[str] = None
    obligations: List[str] = []


class TechnicalTestProfile(BaseModel):
    provider: Optional[str] = None
    model_name: Optional[str] = None
    custom_endpoint: Optional[str] = None
    test_scope: Optional[str] = None
    user_api_key_supplied: bool = False
...
def assess_risk(request: RiskAssessmentRequest):
    ...
    try:
        ...
        result["evidence_summary"] = {
            "mode": "declaration_based",
            "engines": [],
            "scores": request.evidence_layer.model_dump(),
            "technical_test_profile": request.technical_test_profile.model_dump(),
        }
        return result
...
def assess_risk_full(request: RiskAssessmentRequest):
    ...
    try:
        ...
        result = run_assessment_with_tests(
            intake,
            run_owasp=True,
            run_promptfoo=scope in ["standard", "full"],
            run_compl_ai=scope == "full",
            run_lm_eval=scope == "full",
        )
        return result
